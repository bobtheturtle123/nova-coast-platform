...
export default function TenantBookStep1Client({ slug, tenantId, tenantName, catalog }) {
  ...
  const { packageId, serviceIds, setPackage, toggleService, hasSelections, setTenant } =
    useBookingStore();
          ...
          {packages.map((pkg) => {
            ...
            return (
              <button key={pkg.id} onClick={() => setPackage(pkg.id)}
                ...
                <p className={clsx("font-display text-3xl mb-3", selected ? "text-gold" : "text-navy")}>${pkg.price}</p>
                ...
                return (
                  <button key={svc.id} onClick={() => toggleService(svc.id)}
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className={clsx("font-display text-xl", selected ? "text-navy" : "text-charcoal")}>${svc.price}</p>
