    ...
    <div>
      ...
      <section className="bg-navy text-white py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gold text-xs tracking-[0.2em] uppercase mb-4">Booking and Scheduling</p>
          <h1 className="font-serif text-4xl md:text-5xl font-normal leading-tight mb-5 max-w-3xl">
            Take Bookings Around the Clock, Deposit Included
          </h1>
          <p className="text-white/55 text-lg max-w-2xl leading-relaxed mb-8">
            Replace the back-and-forth texts with a professional booking flow that collects the deposit, confirms the appointment, and notifies everyone automatically.
          </p>
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            <div className="mb-4">
              <p className="text-[10px] text-gray-400 uppercase tracking-wide font-semibold mb-3">New Booking</p>
              <div className="space-y-2">
                {[
                  { label: "Package", value: "Standard Photo + Video" },
                  { label: "Property", value: "2847 Coastal Ridge Dr" },
                  { label: "Date", value: "Thursday, May 29 at 10:00 AM" },
                  { label: "Deposit", value: "$99 via Stripe" },
                ].map((row) => (
                  <div key={row.label} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide w-20 flex-shrink-0">{row.label}</p>
                    <p className="text-xs text-gray-800 font-medium">{row.value}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-center mt-4">
              <p className="text-emerald-700 text-xs font-semibold">Booking confirmed. Deposit collected.</p>
              <p className="text-emerald-600 text-[10px] mt-0.5">Confirmation sent to client and photographer.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* HOW IT WORKS */}
      <section className="py-20 px-6 bg-cream border-y border-gray-100">
        <div className="max-w-4xl mx-auto">
      ...
      <section className="bg-cream py-12 px-6 border-y border-gray-100">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <Link href="/blog/automate-real-estate-photography-business" className="text-sm text-gray-600 border border-gray-200 rounded-xl px-4 py-2 hover:border-navy/30 hover:text-navy transition-colors bg-white">
