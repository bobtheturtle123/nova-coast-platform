...
export async function POST(req, { params }) {
  ...
  let paymentUrl = null;
  if (amountDue > 0) {
    // Fail closed: a payable reminder link requires a verified Connect account.
    ...
    try {
      ...
      console.error(`[send-reminder] payment blocked — tenant=${ctx.tenantId} reason=${e?.reason || e?.message}`);
    ...
    try {
      ...
      const { paymentIntentData } = await buildConnectedSessionPaymentData({
        ...
        metadata: { bookingId: params.id, tenantId: ctx.tenantId, type: paymentType },
      });
      const session = await stripe.checkout.sessions.create(
        { ...sessionParams, payment_intent_data: paymentIntentData },
        { idempotencyKey: `rem_${params.id}_${paymentType}_${Math.round(amountDue * 100)}` }
      );
