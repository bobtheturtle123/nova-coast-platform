...
const NAV = [
  ...
  { href: "/dashboard/galleries",  label: "Galleries",    icon: "🖼️" },
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (!token.claims.tenantId) { router.push("/onboarding"); return; }
      setUser(u);
    ...
    <div className="min-h-screen bg-cream flex">
      ...
      <aside className="w-56 bg-navy flex-shrink-0 flex flex-col">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-gold/70 text-xs font-body mt-0.5">Dashboard</p>
        </div>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          <button
            ...
            className="text-xs text-white/40 hover:text-white/70 font-body transition-colors block w-full text-left">
            Sign out
          </button>
        </div>
      </aside>
