...
export default function DashboardLayout({ children }) {
  ...
  const Sidebar = () => (
    <aside className="w-56 bg-navy flex flex-col h-full border-r border-white/5">
      {/* Brand */}
      <div className="px-5 pt-6 pb-5">
        <p className="font-display text-white text-base tracking-[0.12em] uppercase">NovaOS</p>
        {tenantName && (
          <p className="text-white/35 text-xs font-body mt-0.5 truncate tracking-wide">{tenantName}</p>
        )}
      </div>

      {/* Nav */}
      <nav className="px-3 space-y-px flex-1 overflow-y-auto pb-4">
        {NAV.map((item) => {
          const active = isActive(item);
          return (
            <Link key={item.href} href={item.href}
              onClick={() => setSidebarOpen(false)}
              className={`flex items-center gap-3 px-3 py-2 rounded-sm text-[13px] font-body transition-all duration-150 ${
                active
                  ? "bg-white/12 text-white"
                  : "text-white/45 hover:text-white/80 hover:bg-white/6"
              }`}>
              <span className={`flex-shrink-0 transition-opacity ${active ? "opacity-100" : "opacity-60"}`}>
                {item.icon}
              </span>
              <span className="tracking-wide">{item.label}</span>
              {active && (
                <span className="ml-auto w-1 h-3 rounded-full bg-gold flex-shrink-0" />
              )}
            </Link>
          );
        })}
      ...
      <div className="px-4 py-4 border-t border-white/8">
        <div className="text-[11px] text-white/30 truncate mb-2 tracking-wide">{user?.email}</div>
        <button
