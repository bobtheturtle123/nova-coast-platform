...
export function calculatePlatformFee(amountCents, planId) {
  ...
  // Fee-exempt plans (unlimited/lifetime: 0 bps) pay nothing — the minimum
  // floor must not resurrect a fee for them.
  if (!plan.transactionFeeBps) return 0;
  const calculated = Math.round(cents * (plan.transactionFeeBps / 10000));
