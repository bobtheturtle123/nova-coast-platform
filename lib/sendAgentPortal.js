  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:40px 20px;background:#fff">
        ...
        <p style="color:#888;font-size:13px;margin:20px 0 8px">Want to track all your listings in one place?</p>
        <a href="${getAppUrl()}/${tenant.slug}/agent/login"
           style="display:inline-block;border:1px solid #ddd;color:#555;font-family:sans-serif;
                  font-size:13px;padding:10px 24px;border-radius:4px;text-decoration:none">
          Create a free account →
        </a>
        <p style="color:#ccc;font-size:11px;margin:16px 0 0">
          Sign in anytime at <a href="${getAppUrl()}/${tenant.slug}/agent/login" style="color:#aaa">${getAppUrl()}/${tenant.slug}/agent/login</a>
        </p>
