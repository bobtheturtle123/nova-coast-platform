...
// Strip HTML + decode the few entities Aryeo descriptions use. Block-level tags
// are turned into line breaks and list items into "- " bullets so the imported
// text keeps its paragraph/bullet structure (rendered by our RichText component)
// instead of collapsing into one long run-on line.
...
export function mapProductsToServices(products, categories = [], tierNames = []) {
  ...
  for (const p of products || []) {
    ...
    const description = cleanHtml(p.description || p.summary || "").slice(0, 20000);
