  ...
  for (const p of products || []) {
    ...
    if (variants.length > 1) {
      ...
      variants.forEach((v, i) => {
        ...
        if (vPrice <= 0) return;
        // Key onto the studio's configured tier names (by order) so the prices
        // populate the tier grid and apply at booking; fall back to the variant
        // name only if there's no tier at this position.
        const key = (tierNames[i] || String(v.title || v.name || `Option ${i + 1}`).trim()).slice(0, 40);
        priceTiers[key] = vPrice;
        variantNames.push(String(v.title || v.name || `Option ${i + 1}`));
      });
