...
// entered the demo workspace (persisted in sessionStorage so navigation within
// the tab keeps demo on). sessionStorage is per-tab, so a real user's other tabs
// are never affected, and "Exit demo" fully clears it.
//
// NOTE: entering the demo is an EXPLICIT act (visiting /demo or a ?demo=1 URL),
// so demo wins for this tab even when a real user happens to be signed in — that
// lets an owner preview the public demo. A lingering flag can't hijack a real
// login because every sign-in / sign-up path calls exitDemo() first (see
// app/auth/login + register), and a real session ending also clears it.
...
// Products page catalog — intentionally a small, GENERIC set (drone / video /
// photography) so the demo never mirrors the real studio's live product list.
// Ids reuse the svc_* keys above so ordered items on the listing detail page
// still resolve to a name.
export const DEMO_PRODUCT_SERVICES = [
  { id: "svc_drone", name: "Drone Photos",     price: 149, active: true, tagline: "Aerial exterior coverage", description: "FAA-licensed aerial stills showcasing the property and neighborhood." },
  { id: "svc_video", name: "Video",            price: 299, active: true, tagline: "Cinematic walkthrough",     description: "Gimbal-stabilized video tour with licensed music." },
  { id: "svc_hdr",   name: "HDR Photography",  price: 179, active: true, tagline: "Bright, true-to-life interiors", description: "Professionally lit and edited HDR stills, delivered next morning." },
];
...
// No demo listing carries a revision request — the sample is kept clean so the
// listings page shows no "revision" badges.
...
// The one listing a demo visitor can actually open, keyed by street address so
// it stays stable no matter how the sample list is generated. Every other card
// on the listings page is there for visual completeness only.
export const DEMO_EXAMPLE_ADDRESS = "66 Linda Vista Ter";
...
export function getDemoListings() {
  ...
  // Flag the single openable example (66 Linda Vista Ter) so the card can badge
  // it "Demo listing" and only that card links through to a working detail page.
  const example =
    listings.find((l) => (l.address || "").startsWith(DEMO_EXAMPLE_ADDRESS)) || listings[0];
  listings.forEach((l) => { l.demoExample = !!example && l.id === example.id; });
...
// ── single listing detail (the one openable demo example) ─────────────────────
// Returns everything the listing detail page loads (booking, gallery, catalog,
// product-name index, team) so the page renders fully without any API/auth.
export function getDemoListingDetail(id) {
  const base = buildDemoListings().find((l) => l.id === id);
  if (!base) return null;

  const productNames = {};
  [...DEMO_PACKAGES, ...DEMO_SERVICES, ...DEMO_ADDONS].forEach((p) => { productNames[p.id] = p.name; });

  // A rich, "delivered & paid" booking so the whole detail page has something to
  // show (agreement signed, property site published, services ordered).
  const nowIso = new Date().toISOString();
  const booking = {
    ...base,
    isListing: true,
    clientPhone: "(858) 555-0142",
    clientCompany: "Coastal Realty",
    squareFootage: 3200,
    propertyType: "Single Family",
    serviceIds: ["svc_drone"],
    addonIds: ["add_twilight"],
    promoDiscount: 0,
    paidInFull: true,
    depositPaid: true,
    balancePaid: true,
    remainingBalance: 0,
    // Signed service agreement (so the agreement card shows a valid record).
    contractSigned: true,
    contractSignerName: base.clientName,
    contractAcceptanceMethod: "checkbox",
    contractSignedAt: nowIso,
    contractCounterSigned: true,
    contractCounterSignedAt: nowIso,
    contractCounterSignedBy: DEMO_TENANT.businessName,
    contractText: "This is a sample service agreement shown in the KyoriaOS demo. In your live workspace this holds your own terms, which clients accept before their gallery unlocks.",
    propertyWebsite: {
      published: true,
      address: base.fullAddress,
      beds: 4,
      baths: 3,
      agentName: base.clientName,
      agentEmail: base.clientEmail,
      agentPhone: "(858) 555-0142",
      agentBrokerage: "Coastal Realty",
      features: ["Ocean views", "Chef's kitchen", "Primary suite with spa bath"],
    },
  };

  const media = COVERS.slice(0, 8).map((url, i) => ({
    id: `demo_m_${i}`,
    url,
    fileType: "image/jpeg",
    seqName: `${base.fullAddress?.split(",")[0] || "photo"}-${String(i + 1).padStart(2, "0")}.jpg`,
  }));
  ...
  const gallery = {
    ...
    floorPlans: [{ id: "fp_1", url: COVERS[9], name: "Main Level Floor Plan" }],
    attachedFiles: [],
    matterportUrl: "https://my.matterport.com/show/?m=demo",
    videoUrl: "",
    delivered: true,
    unlocked: true,
    deliveredAt: nowIso,
  ...
  return { booking, gallery, catalog, productNames, team: DEMO_TEAM };
}
...
export function getDemoProducts() {
  return {
    items: {
      packages:  [],
      services:  DEMO_PRODUCT_SERVICES,
      addons:    [],
      retainers: [],
    },
    teamMembers: DEMO_TEAM,
