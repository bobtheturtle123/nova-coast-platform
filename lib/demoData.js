...
// The one listing a demo visitor can actually open. Every other card on the
// listings page is there for visual completeness only (see getDemoListings).
export const DEMO_EXAMPLE_LISTING_ID = "demo_l_0";
...
export function getDemoListings() {
  ...
  // Flag the single openable example so the card can badge it "Demo listing"
  // and only that card links through to a working detail page.
  listings.forEach((l) => { l.demoExample = l.id === DEMO_EXAMPLE_LISTING_ID; });
