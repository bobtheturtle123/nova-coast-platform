...
export function isDemo() {
  ...
  try {
    // A real signed-in user ALWAYS wins — demo can never apply to a real session
    // (even if a stale ky_demo flag lingers in the tab from an earlier visit).
    if (auth?.currentUser) { try { window.sessionStorage.removeItem("ky_demo"); } catch {} return false; }
