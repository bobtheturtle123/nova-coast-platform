...
export async function getTravelFee(originZip, destinationAddress, config = {}) {
  const {
    enabled     = true,
    mode        = "perMile",  // "perMile" | "flat" | "zones"
    flatFee     = 0,
    freeRadius  = 20,
    ratePerMile = 1.5,
    maxRadius   = 0,
  ...
  // Flat fee: charge exactly the configured amount (which may be $0) — never
  // run the per-mile calculation. This was the bug: a $0 flat fee was being
  // overridden by a distance-based charge.
  if (mode === "flat") {
    return { fee: Math.max(0, Number(flatFee) || 0), miles: 0, withinRange: true };
  }

  // Zone-based fees are applied via service areas, not distance — don't add a
  // per-mile charge on top.
  if (mode === "zones") {
