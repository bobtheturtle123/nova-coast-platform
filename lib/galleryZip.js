...
export async function buildGalleryZipBuffer(gallery, opts = {}) {
  const { format = "web", slug, token, bookingId } = opts;
  ...
  // Videos: bundle the small ones directly into Videos/ so the client gets them
  // inside the package. Oversized videos can't be buffered safely here (memory),
  // so they stay as separate direct downloads and are listed in
  // Links/Video-Download-Links.txt.
  ...
  for (const fp of floorPlans) {
    try {
      ...
      if (format === "package") {
        archive.append(buffer, { name: `${ROOT}/Floor Plans/Print Ready/${fpName}` });
        if (isImg) {
          ...
          archive.append(buffer, { name: `${ROOT}/Floor Plans/Web Ready/${fpName}` });
        }
      } else {
        archive.append(buffer, { name: `${ROOT}/Floor Plans/${fpName}` });
      }
    } catch { /* skip */ }
  }
