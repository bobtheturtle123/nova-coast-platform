...
export function buildLinkFiles(gallery, { slug, token, bookingId, separateVideos = [] } = {}) {
  ...
  // Property website / brochure
  if (slug && bookingId) {
    const siteLines = [];
    if (gallery.showPropertyWebsiteLink !== false) {
      siteLines.push(`Property Website: ${appUrl}/${slug}/property/${bookingId}`);
    }
    siteLines.push(`Property Brochure: ${appUrl}/${slug}/property/${bookingId}/brochure`);
...
export async function buildGalleryZipBuffer(gallery, opts = {}) {
  ...
  for (const fp of floorPlans) {
    try {
      ...
      if (format === "package") {
        archive.append(buffer, { name: `${ROOT}/Print Ready/Floor Plans/${fpName}` });
        if (isImg) {
          ...
          archive.append(buffer, { name: `${ROOT}/Web Ready/Floor Plans/${fpName}` });
        }
      } else {
