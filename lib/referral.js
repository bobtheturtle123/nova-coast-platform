...
export async function applyReferralCredit(tenantId, stripeCustomerId, amountCents) {
  // Stripe credit must succeed before we record it — let errors propagate so
  // triggerReferralReward can exit without marking the referral "rewarded".
  if (stripeCustomerId) {
    await stripe.customers.createBalanceTransaction(stripeCustomerId, {
      amount:      -amountCents,
      currency:    "usd",
      description: "KyoriaOS referral credit - $20",
    });
  }
