...
function cfg() {
  // Trim defensively — a trailing newline/space pasted into an env var is the
  // most common cause of a "correct" secret being rejected as invalid_client.
  return {
    clientId:     (process.env.DROPBOX_CLIENT_ID || "").trim(),
    clientSecret: (process.env.DROPBOX_CLIENT_SECRET || "").trim(),
...
// Safe diagnostic: lengths only (never the secret values) + the redirect URI,
// which is not sensitive. Helps spot empty/whitespace/wrong-scope env issues.
export function clientDebug() {
  const c = cfg();
  return { idLen: c.clientId.length, secretLen: c.clientSecret.length, redirectUri: c.redirectUri };
}
...
// Dropbox-API-Arg is sent as an HTTP header, which must be ASCII. Any non-ASCII
// char in a path/filename (accents, emoji, …) must be escaped as \uXXXX, or the
// request is rejected (and Node's fetch throws "invalid header value") — a
// common cause of imports failing on files with accented names.
function apiArg(obj) {
  return JSON.stringify(obj).replace(/[-￿]/g,
    (c) => "\\u" + c.charCodeAt(0).toString(16).padStart(4, "0"));
}

// Fetch a small JPEG thumbnail for an image file (for the import picker).
// Returns the JPEG bytes; throws if Dropbox can't thumbnail the file.
export async function getThumbnail(tenantId, path, size = "w256h256") {
  const token = await getValidAccessToken(tenantId);
  const res = await fetch(`${CONTENT}/files/get_thumbnail_v2`, {
    ...
    headers: {
      ...
      "Dropbox-API-Arg": apiArg({
        ...
        format: "jpeg", size, mode: "strict",
      }),
    },
  });
  if (!res.ok) throw new Error(`Dropbox thumbnail error ${res.status}: ${await res.text()}`);
