...
export async function deleteGalleryPreviews(galleryId) {
  if (!galleryId || !process.env.R2_BUCKET_NAME) return 0;
  try {
    ...
    do {
      const list = await s3.send(new ListObjectsV2Command({ Bucket, Prefix: `previews/${galleryId}/`, ContinuationToken }));
