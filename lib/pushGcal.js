...
// Resolves which calendar a booking's photographer maps to, tolerant of how
// photographerId was stored: "__owner__" or the owner's uid → the owner's
// calendar (on the tenant doc); a team doc id or a team member's uid → that
// member's calendar. Returns { gcal, tokenRef, tokenPrefix } or { error, skipped }.
async function resolvePhotographerCalendar(tenantRef, tenantData, photographerId) {
  const isOwner = photographerId === "__owner__"
    || (tenantData.ownerUid && photographerId === tenantData.ownerUid);
  if (isOwner) {
    return { gcal: tenantData.ownerGoogleCalendar, tokenRef: tenantRef, tokenPrefix: "ownerGoogleCalendar" };
  }
  // Team member: try the doc id directly, then fall back to matching their uid
  // (the listing's "Assign me" stores an auth uid, not the team doc id).
  let memberDoc = await tenantRef.collection("team").doc(photographerId).get();
  if (!memberDoc.exists) {
    const q = await tenantRef.collection("team").where("uid", "==", photographerId).limit(1).get();
    if (!q.empty) memberDoc = q.docs[0];
  }
  if (!memberDoc.exists) return { error: "Photographer not found", skipped: true };
  const memberData = memberDoc.data();
  if (memberData.calendarPrefs?.writeBookings === false) return { error: "Calendar write disabled", skipped: true };
  return { gcal: memberData.googleCalendar, tokenRef: memberDoc.ref, tokenPrefix: "googleCalendar" };
}
...
export async function pushBookingToGcal(tenantId, bookingId) {
  ...
  const resolved = await resolvePhotographerCalendar(tenantRef, tenantData, booking.photographerId);
  if (resolved.error) return { ok: false, skipped: resolved.skipped, reason: resolved.error };
  const { gcal, tokenRef, tokenPrefix } = resolved;
  ...
  async function upsertEvent(eventBody, existingEventId) {
    ...
    if (!res.ok) {
      ...
      const msg = err.error?.message || "Google Calendar API error";
      // Token was granted without write access (connected before the events
      // scope existed, or the calendar permission was declined at consent).
      if (res.status === 403 && /scope/i.test(msg)) {
        throw new Error("Google Calendar is connected without write access. Disconnect and reconnect it, and allow calendar access when Google asks.");
      }
      throw new Error(msg);
    }
