...
export function buildBookingIcs({ booking, tenant, shootDate, shootTime, uid, sequence = 0, method = "PUBLISH", attendeeEmail = null, attendeeName = null, description = null }) {
  ...
  const orgEmail = tenant?.email || "no-reply@mail.kyoriaos.com";
  ...
  const lines = [
    ...
    // ORGANIZER/ATTENDEE only for REQUEST (a formal invite). For PUBLISH we
    // omit them so any recipient can just add the event — matching the
    // create-flow .ics that loads reliably in Gmail.
    wantsAttendee ? `ORGANIZER;CN=${esc(biz)}:mailto:${orgEmail}` : null,
