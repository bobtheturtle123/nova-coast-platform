...
// Owner/super-admin email allowlist. These accounts are super-admins by email,
// without needing the role claim set — so the owner always has access.
const BUILTIN_SUPERADMIN_EMAILS = ["complexdesign123@gmail.com"];
function getAllowedEmails() {
  const raw = process.env.SUPERADMIN_EMAILS || "";
  return [...BUILTIN_SUPERADMIN_EMAILS, ...raw.split(",").map((s) => s.trim())]
    .map((s) => s.toLowerCase()).filter(Boolean);
}
function emailAllowed(decoded) {
  return !!decoded?.email && getAllowedEmails().includes(String(decoded.email).toLowerCase());
}
...
export async function isSuperAdmin(req) {
  ...
  try {
    ...
    if (emailAllowed(decoded)) return true;
...
export async function getSuperAdminCtx(req) {
  ...
  try {
    const decoded = await adminAuth.verifyIdToken(token);
    if (emailAllowed(decoded)) return decoded;
