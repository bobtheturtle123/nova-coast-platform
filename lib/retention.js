...
export function isVideo(m = {}) {
  const t = (m.fileType || "").toLowerCase();
  if (t) return t.startsWith("video/");
  const key = (m.key || m.url || "").toLowerCase();
  return /\.(mp4|mov|webm|m4v|avi|mkv)$/.test(key);
}
...
// Full-resolution VIDEO originals eligible for cleanup. Like photos, eligible
// only once the gallery is past its 1-year window. The full-res original is
// removed only after a 1080p web version exists (so playback still works); the
// retention job generates the web version first if one is missing.
export function eligibleVideoOriginals(gallery, now = Date.now()) {
  if (!isPastRetention(gallery, now)) return { items: [], bytes: 0 };
  const items = (gallery.media || []).filter(
    (m) => isVideo(m) && !m.originalRemoved && m.webVideoStatus !== "skipped_large"
  );
  const bytes = items.reduce((s, m) => s + (Number(m.size) || 0), 0);
  return { items, bytes };
}
