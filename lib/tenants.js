...
export async function getTenantCatalog(tenantId) {
  ...
  return deepSerialize({
    ...
    packages:      pkgSnap.docs.map((d) => d.data()).filter((p) => p.active !== false),
    services:      svcSnap.docs.map((d) => d.data()).filter((s) => s.active !== false),
    addons:        addonSnap.docs.map((d) => d.data()).filter((a) => a.active !== false),
    retainers:     retainerSnap.docs.map((d) => d.data()).filter((r) => r.active !== false),
    pricingConfig,
    bookingConfig,
    photographers,
