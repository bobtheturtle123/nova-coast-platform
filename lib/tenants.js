...
// In-process TTL cache for tenant lookups.
// Eliminates duplicate reads inside cron loops and notification batches.
// Per-instance only (serverless) — 5-minute TTL prevents stale data accumulating.
const _tenantCache = new Map();
const TENANT_TTL = 5 * 60 * 1000;

export function _invalidateTenantCache(tenantId) {
  _tenantCache.delete(tenantId);
}

/**
 * Fetch tenant by ID (cached — 5 min TTL).
 */
export async function getTenantById(tenantId) {
  ...
  const tenant = serializeTenant(doc.id, doc.data());
  _tenantCache.set(tenantId, { data: tenant, expiresAt: Date.now() + TENANT_TTL });
  return tenant;
}
...
export async function createTenant({
  ...
  // Atomic per-owner guard: a Firestore transaction on tenantOwners/{uid}
  // ensures that concurrent registrations (double-click, retries, two
  // serverless instances) all resolve to ONE tenantId instead of creating
  // duplicate tenants. The transaction is the source of truth for "does this
  // account already own a tenant".
  const ownerGuardRef = adminDb.collection("tenantOwners").doc(uid);
  const { tenantId, alreadyOwned } = await adminDb.runTransaction(async (tx) => {
    const g = await tx.get(ownerGuardRef);
    if (g.exists && g.data().tenantId) {
      return { tenantId: g.data().tenantId, alreadyOwned: true };
    }
    const newId = adminDb.collection("tenants").doc().id;
    tx.set(ownerGuardRef, { uid, tenantId: newId, createdAt: new Date() });
    return { tenantId: newId, alreadyOwned: false };
  });

  // Idempotent: if this owner already had a tenant, don't re-seed / overwrite —
  // just make sure the claim is set and return it.
  if (alreadyOwned) {
    const existing = await adminDb.collection("tenants").doc(tenantId).get();
    if (existing.exists) {
      await adminAuth.setCustomUserClaims(uid, { role: "owner", tenantId }).catch(() => {});
      return tenantId;
    }
    // Guard existed but tenant doc doesn't (interrupted first run) — fall
    // through and create it under the reserved id.
  }
...
export async function getTenantCatalog(tenantId) {
  ...
  const [tenantDoc, pkgSnap, svcSnap, addonSnap, retainerSnap] = await Promise.all([
    tenantRef.get(),
    tenantRef.collection("packages").get(),
    tenantRef.collection("services").get(),
    tenantRef.collection("addons").get(),
    tenantRef.collection("retainers").get(),
  ]);
  ...
  let photographers = [];
  if (bookingConfig?.availability?.allowAgentPhotographerSelection) {
    const teamSnap = await tenantRef.collection("team").get();
    photographers = teamSnap.docs
      .map((d) => ({ id: d.id, name: d.data().name, color: d.data().color || null, photoUrl: d.data().photoUrl || null, role: d.data().role || "photographer", active: d.data().active !== false }))
      .filter((m) => m.name && m.active && (!m.role || m.role === "photographer"));
  }

  // Respect the studio's chosen display order (sortOrder), then fall back to
  // creation time for any items that predate ordering.
  const byOrder = (a, b) => {
  ...
  // Resolve a package's included-service IDs to names using ALL services
  // (including inactive ones), so the booking page never shows raw IDs.
  const serviceNameById = {};
  svcSnap.docs.forEach((d) => { const s = d.data(); if (s?.id) serviceNameById[s.id] = s.name; });
  const withIncludeNames = (p) => ({
    ...p,
    includeNames: (p.includes || []).map((id) => serviceNameById[id]).filter(Boolean),
  });

  const addonList = addonSnap.docs.map((d) => d.data()).filter((a) => a.active !== false).sort(byOrder);

  // When the tenant charges for connecting a custom property-website domain,
  // surface it as a selectable add-on/service so agents can opt in and are
  // charged the set fee. Synthetic (not stored in the addons collection).
  const domainCharge = tenantData.customDomainCharge;
  if (domainCharge?.enabled && Number(domainCharge.amount) > 0) {
    addonList.push({
      id:          "custom-domain",
      name:        "Custom Website Domain",
      description: "Connect your own domain (e.g. listings.yourbrokerage.com) to this property website. A one-time setup fee applies.",
      price:       Math.max(0, Math.round(Number(domainCharge.amount) || 0)),
      active:      true,
      _customDomain: true,
    });
  }
  ...
  return deepSerialize({
    // Only active products are offered on the public booking page — toggling a
    // product off in Products hides it everywhere clients book.
    packages:      pkgSnap.docs.map((d) => d.data()).filter((p) => p.active !== false).map(withIncludeNames).sort(byOrder),
    services:      svcSnap.docs.map((d) => d.data()).filter((s) => s.active !== false).sort(byOrder),
    addons:        addonList,
    retainers:     retainerSnap.docs.map((d) => d.data()).filter((r) => r.active !== false).sort(byOrder),
    pricingConfig,
    bookingConfig,
    photographers,
    country:  tenantData.country  || "US",
    tempUnit: tenantData.tempUnit || "F",
  });
}
...
export const PLANS = {
  solo:    { name: "Solo",   price: 39,  activeListings: 15,  teamMembers: 1,   sms: false },
  studio:  { name: "Studio", price: 89,  activeListings: 50,  teamMembers: 5,   sms: true  },
  pro:     { name: "Pro",    price: 179, activeListings: 125, teamMembers: 15,  sms: true  },
  scale:   { name: "Scale",  price: 349, activeListings: 300, teamMembers: 999, sms: true  },
  starter: { name: "Solo",   price: 39,  activeListings: 15,  teamMembers: 1,   sms: false },
};
