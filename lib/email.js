...
async function safeSend(payload) {
  ...
  return new Resend(key).emails.send(payload);
}
...
export async function sendWelcomeEmail({ email, businessName, slug }) {
  ...
  const fromDomain = process.env.RESEND_FROM_DOMAIN || "nova-os.app";
  await safeSend({
    from: `KyoriaOS <hello@${fromDomain}>`,
    to: email,
...
function tenantFrom(tenant) {
  ...
  const domain = process.env.RESEND_FROM_DOMAIN || "nova-os.app";
