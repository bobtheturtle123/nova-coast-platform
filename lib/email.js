...
export async function sendGalleryDelivery({ booking, galleryToken, tenant, subject, note, to, cc, websiteUrl, tourUrl }) {
  ...
  const emailSubject = subject
    || (tenant.emailTemplate?.subject ? applyPlaceholders(tenant.emailTemplate.subject, vars) : null)
    || `Your media is ready - ${address}`;

  // note may be HTML from rich text editor; fall back to tenant template body
  const rawNote = note || (tenant.emailTemplate?.body
    ? applyPlaceholders(tenant.emailTemplate.body, vars)
    : "");
  ...
  const extraLinks = [
    websiteUrl ? `<tr><td style="padding:8px 0;border-bottom:1px solid #f0f0f0;color:#888;font-size:13px">Property Website</td><td style="padding:8px 0;border-bottom:1px solid #f0f0f0"><a href="${websiteUrl}" style="color:${tenant.branding.primaryColor}">${websiteUrl}</a></td></tr>` : "",
    tourUrl    ? `<tr><td style="padding:8px 0;color:#888;font-size:13px">3D Tour</td><td style="padding:8px 0"><a href="${tourUrl}" style="color:${tenant.branding.primaryColor}">${tourUrl}</a></td></tr>` : "",
  ].filter(Boolean).join("");
