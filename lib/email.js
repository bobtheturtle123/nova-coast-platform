...
export async function sendBookingCreatedNotifications({ booking, tenant, adminEmail }) {
  const {
    clientName, clientEmail, address, preferredDate, preferredTime,
    totalPrice, notes, photographerEmail, photographerName,
  } = booking;
  ...
  const shootDate = preferredDate
    ...
    : "To be confirmed";
  const shootTime = preferredTime || "Time to be confirmed";
  ...
  const detailsTable = `
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
      ...
      <tr><td style="padding:10px 0;border-bottom:1px solid #eee;color:#888;font-size:13px">Time</td>
          <td style="padding:10px 0;border-bottom:1px solid #eee;font-weight:500">${shootTime}</td></tr>
  ...
  if (adminEmail) {
    sends.push(resend.emails.send({
      ...
      html:    emailWrapper(`
        ...
        <p style="color:#555;margin:0 0 4px"><strong>${clientName}</strong> · ${clientEmail}</p>
  ...
  // 3 — Photographer notification
  if (photographerEmail) {
    const photographerCalICS = icsContent
      ? generateCalendarICS({
          summary:     `📷 ${address}`,
          description: `Client: ${clientName} (${clientEmail})\nTotal: $${totalPrice || 0}${notes ? "\n\nNotes: " + notes : ""}`,
          location:    address,
          startISO:    preferredDate && preferredTime ? `${preferredDate}T${preferredTime}:00` : null,
          durationMinutes: tenant.bookingConfig?.availability?.defaultDuration || 120,
        })
      : null;
    ...
    sends.push(resend.emails.send({
      ...
      html:    emailWrapper(`
        <h2 style="font-family:Georgia,serif;color:${primary};font-size:22px;margin:0 0 8px">
          You've been assigned a shoot.
        </h2>
        <p style="color:#555;margin:0 0 24px">Hi ${photographerName || "there"},</p>
        ${detailsTable}
        ${notes ? `<p style="color:#888;font-size:13px;font-style:italic;margin:16px 0 0">"${notes}"</p>` : ""}
      `, tenant),
      ...(photographerCalICS ? { attachments: [{ filename: "shoot-assignment.ics", content: Buffer.from(photographerCalICS).toString("base64") }] } : {}),
    }));
  }
