  ...
  const ctaButton = paymentUrl
    ? `<a href="${paymentUrl}"
       ...
       <p style="color:#aaa;font-size:12px;margin:16px 0 0">
         Or copy this link: <a href="${paymentUrl}" style="color:#888;word-break:break-all">${paymentUrl}</a>
       </p>`
    : galleryToken
...
export async function sendDepositRequestEmail({ booking, depositUrl, tenant }) {
  ...
  await safeSend({
    ...
    html: emailWrapper(`
      ...
      <p style="color:#aaa;font-size:12px;margin:16px 0 0">
        Or copy this link: <a href="${depositUrl}" style="color:#888;word-break:break-all">${depositUrl}</a>
      </p>` : ""}
    `, tenant),
  });
}
