...
// GALLERY DELIVERY
// ─────────────────────────────────────────────────────────────────────────────
export async function sendGalleryDelivery({ booking, galleryToken, tenant, subject, note }) {
  const { clientName, clientEmail, address, remainingBalance } = booking;
  const from = tenantFrom(tenant);
  const galleryUrl = `${APP_URL}/${tenant.slug}/gallery/${galleryToken}`;
  const emailSubject = subject || `Your media is ready — ${address}`;
  const noteHtml = note
    ? `<p style="color:#555;margin:0 0 20px;font-style:italic">${note}</p>`
    : "";
